...
仿照慕课网开发的教育平台，利用现在比较流行的 mono 模式，也就是把多个 project 放在一个 repository 中，这样就方便我们系统内的一些公共模块的调用，而不用发布 npm 包。下面对这个 repository 结构做简要介绍。

### supply 文件夹

supply 文件夹是项目的主要文件夹，所有的系统都在这个文件夹中开发，目前来说，这个项目暂时就是以下三个系统组成。其中，前端网站和后端接口是最重要的，需要先开发。
